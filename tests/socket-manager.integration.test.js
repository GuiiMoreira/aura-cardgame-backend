...
function criarCartasMestras(ids) {
    return Object.fromEntries(ids.map((id) => [id, { Nome: `Carta ${id}`, C: 0, M: 0, O: 0, A: 0, Força: 200, Vida: 100 }]))
}
                ...
                if (name === 'partidas_ativas') {
                    const docs = Object.entries(state.partidas_ativas)
                        .filter(([, partida]) => {
                            ...
                            if (field === 'expiresAt' && op === '<=') return partida.expiresAt && partida.expiresAt <= value;
                            return true;
                        })
                        .map(([id, partida]) => ({
                            id,
...
async function criarAmbiente() {
    ...
    const criarCliente = (uid) => ioc(`http://127.0.0.1:${port}`, {
        auth: { token: uid },
